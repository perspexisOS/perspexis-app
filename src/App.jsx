...
export default function PerspexisCore() {
  ...
  const [identityMode, setIdentityMode] = useState(DEFAULT_IDENTITY ? "view" : "setup");
  const [people, setPeople] = useState(DEFAULT_PEOPLE);
  const [gaps, setGaps] = useState(DEFAULT_GAPS);
  const [rhythm, setRhythm] = useState(null);
  const [rhythmMode, setRhythmMode] = useState("setup");

  const updateRole = (role, owns) => setPeople(p => p.map(r => r.role === role ? { ...r, owns } : r));
  const updateRhythm = (updated) => setRhythm(updated);
  ...
  // Show onboarding screen if not yet set up
  if (!onboarded) {
    return (
      <OnboardingScreen onStart={(name, type) => {
        setOrgName(name);
        setOnboarded(true);
        setIdentityMode("setup");
      }} />
    );
  }
          ...
          {LAYERS.map(l => active === l.id && (
            <div key={l.id}>
              ...
              {l.id === "identity" && (
                !identity
                  ...
                  : identityMode === "edit" || identityMode === "setup"
                    ? <IdentityEdit data={identity || {mission:"",guiding:"",vision_north:"",vision_phase:"",values:[{name:"",desc:""},{name:"",desc:""},{name:"",desc:""},{name:"",desc:""}],positioning:""}} onSave={d => { setIdentity(d); setIdentityMode("view"); }} onCancel={() => identity && setIdentityMode("view")} isNew={!identity} />
              ...
              {l.id === "rhythm" && (rhythm && rhythmMode === "view"
                ...
                : <RhythmSetup onDone={d => { setRhythm(d); setRhythmMode("view"); }} />
              )}
            </div>
          ))}
        </div>
