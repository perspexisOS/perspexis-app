...
function AuthScreen({ onAuth }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: DISPLAY }}>
      ...
      <div style={{ width: "100%", maxWidth: 420, padding: "0 24px", animation: "fadeUp 0.4s ease both" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ fontSize: 32, marginBottom: 8 }}>◈</div>
          <h1 style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: 24, color: "var(--text-primary)", margin: "0 0 6px" }}>perspexis</h1>
          <p style={{ fontFamily: MONO, fontSize: 10, color: "var(--accent)", textTransform: "uppercase", letterSpacing: 2, margin: 0 }}>Clarity. Alignment. Momentum.</p>
        </div>
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: 28 }}>
          <h2 style={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 18, color: "var(--text-primary)", margin: "0 0 20px" }}>
          ...
          {error && <p style={{ fontSize: 12, color: "var(--accent)", fontFamily: BODY, margin: "0 0 12px" }}>{error}</p>}
          {message && <p style={{ fontSize: 12, color: "#2EC4B6", fontFamily: BODY, margin: "0 0 12px" }}>{message}</p>}
          <button onClick={handleSubmit} disabled={loading || !email} style={{ width: "100%", padding: "12px", background: email ? "var(--accent)" : "rgba(255,255,255,0.05)", border: "none", borderRadius: 6, color: email ? "#071827" : "var(--text-secondary)", fontFamily: DISPLAY, fontSize: 13, fontWeight: 700, cursor: email ? "pointer" : "not-allowed", marginBottom: 16 }}>
            {loading ? "Please wait..." : mode === "login" ? "Sign In →" : mode === "signup" ? "Create Account →" : "Send Reset Email →"}
          </button>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
            {mode === "login" && (<><button onClick={() => setMode("signup")} style={{ background: "none", border: "none", color: "var(--accent)", fontFamily: BODY, fontSize: 13, cursor: "pointer" }}>Don't have an account? Sign up</button><button onClick={() => setMode("reset")} style={{ background: "none", border: "none", color: "var(--text-secondary)", fontFamily: BODY, fontSize: 12, cursor: "pointer" }}>Forgot password?</button></>)}
            {mode !== "login" && <button onClick={() => setMode("login")} style={{ background: "none", border: "none", color: "var(--accent)", fontFamily: BODY, fontSize: 13, cursor: "pointer" }}>Back to sign in</button>}
          </div>
        </div>
      </div>
    </div>
