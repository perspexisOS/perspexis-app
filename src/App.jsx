...
// ─── Maven Experience ─────────────────────────────────────────────────────────
function MavenExperience({ user, orgName, orgType, identity, people, gaps, rhythm, active, setActive, completeTutorial }) {
  const [phase, setPhase] = useState(null);
  // null=chat-bubble-only | welcome | tour | done
  ...
  const nextTour = () => {
    ...
    else { setPhase('done'); if (completeTutorial) completeTutorial(); }
  };
  const skipTour = () => { setPhase('done'); if (completeTutorial) completeTutorial(); };
...
{showPricing && <PricingPage user={user} onClose={() => setShowPricing(false)} />}
      ...
      <MavenExperience user={user} orgName={orgName} orgType={orgType} identity={identity} people={people} gaps={gaps} rhythm={rhythm} active={active} setActive={setActive} completeTutorial={completeTutorial} />
      {/* Mobile bottom nav */}
